...
public class NotesList extends Activity {
    ...
    // For logging and debugging用于日志记录和调试
    ...
    /**
     * The columns needed by the cursor adapter游标适配器所需的列
     */
    ...
    /** The index of the title column 标题列的索引*/
    private static final int COLUMN_INDEX_TITLE = 1;

    /**
     * onCreate is called when Android starts this Activity from scratch.
     * 当Android从零开始启动此活动时，将调用onCreate
     */
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // The user does not need to hold down the key to use menu shortcuts.用户不需要按住键来使用菜单快捷键
         ...
         * provider URI.
         * 如果在启动这个活动的意图中没有给出数据，那么当意图过滤器匹配一个主操作时，这个活动就启动了。我们应该使用默认的提供程序URI。
         */
        // Gets the intent that started this Activity.
        Intent intent = getIntent();

        // If there is no data associated with the Intent, sets the data to the default URI, which如果没有与意图关联的数据，则将数据设置为默认URI
         ...
         * 为ListView的上下文菜单激活设置回调。侦听器被设置为这个活动。其效果是，上下文菜单为ListView中的项启用，而上下文菜单由NotesList中的方法处理。
         */
        listView.setOnCreateContextMenuListener(this);

        /* Performs a managed query. The Activity handles closing and requerying the cursor
         * when needed.
         *执行托管查询。该活动在需要时处理关闭和重新请求游标。
         ...
         *以下两个数组在游标的列和视图id之间创建一个“映射”

          *用于列表视图中的项。dataColumns数组中的每个元素表示

          *列名;viewID数组中的每个元素表示一个视图的ID。

           *  SimpleCursorAdapter按升序映射它们，以确定每个列的位置
